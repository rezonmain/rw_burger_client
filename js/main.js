...
function getNumberOfBurgers(url) {
  if (
    readFromCache(url) === null ||
    Date.now() - readFromCache(url).timestamp >= 10000
  ) {
    fetch(url)
...
function showErrorMessage(error) {
  ...
  if (error.startsWith("TypeError: NetworkError")) {
    ...
    errorSpan.innerText =
      "Error de conexión, verifica si estás conectado a una red😓.";
  } else {
    // Show a generic error
    errorSpan.innerText =
      "Se ha producido un error porfavor de refrescar la página🤕.";
  }
  loader.remove();
